...
create table if not exists public.signals (
  ...
  collected_day date not null default (now()::date),
  ttl_days int default 7,
  provider text
);
create unique index if not exists uniq_signals_candidate_signal_day on public.signals (candidate_id, signal_id, collected_day);
